...
c-----------------------------------------------------------------------
      ...
      elseif (((nproc .ge. 70) .and. (nproc .le. 80)).or.(nproc.eq.800)) 
     &     then
        case='WZbbar'
...
c---  as long as anomtgc=.false. too
     ...
     &     .and. ( anomtgc  .eqv. .false.))
        ...
        if ((nproc .le. 75).or.(nproc.eq.800)) then
c-- W^+Z
          nwz=+1
...
c--  75 '  f(p1)+f(p2) --> W^+(-->nu(p3)+mu^+(p4))+Z^0(-->2*(u(p5)+u~(p6)))'
            ...
            l1=l(4)*dsqrt(2d0*xn)
            r1=r(4)*dsqrt(2d0*xn)
         elseif (nproc .eq. 800) then
c--  800 '  f(p1)+f(p2) --> W^-(-->q(p3)+q~(p4))+Z^0(-->e^-(p5)+e^+(p6))' 'N'
c--- note: scattering diagrams are NOT included, only couplings change
...
!------ WE NEED TO ADJUST FOR NLO ME 
!------ If in total cross section mode we remove one dimension, 
!------ Otherwise, for part = lord we have ndim=1, virt ndim =2, real ndim = 4 
!----- Set to 1 here and adjust in NLOME as appropriate 
!      if(total_cr) then 
         ndim_cr=ndim-1 
!       write(6,*) 'Ndim in chooser = ',ndim_cr 
!!     else
!         ndim = 1
!      endif
...
c--- set flags to true unless we're doing W+2 jet or Z+2 jet
 ...
 43   write(6,*) 'problems opening process.DAT'
      stop
 50   WRITE(6,*) 'COULDNT READ PROCESS.DAT'
