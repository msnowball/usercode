         block data vegas_data_cr
         implicit  double precision (a-h,o-z)
         implicit integer (i-n)
         include 'vegas_common_cr.f'
         include 'maxwt_cr.f'
         parameter(mprod=50*mxdim_cr)
         common/bveg2_cr/xi_cr(50,mxdim_cr),si_cr,si2_cr,swgt_cr,
        & schi_cr,ndo_cr,it_cr
c
         data ncall_cr/10000/,itmx_cr/15/,nprn_cr/1000/,acc_cr/-1d0/,
     1   xl_cr/mxdim_cr*0d0/
     2   xu_cr/mxdim_cr*1d0/
c
         data XI_cr/mprod_cr*1d0/
c
         data wtmax_cr/0d0/
c
         end
...
C
C
C        NCALL_CR IS THE NUMBER OF CALLS TO VEGAS.
...
c
         subroutine vegas_cr(fxn,avgi,sd,chi2a)
c
...
c
     ...
     1   dx(mxdim),dt(mxdim),x(mxdim),kg(mxdim),ia(mxdim)
         ...
         if(ndim_cr .gt. mxdim) then
         write(6,*) 'ndim_cr',ndim_cr
         write(6,*) 'mxdim',mxdim
         write(6,*) 'ndim_cr .gt. mxdim'
         stop
         endif
...
c        no initialisation
         ...
         ng=1
         if(mds.eq.0)go to 2
         ng=int((dble(ncall_cr)/2d0)**(1d0/dble(ndim_cr)))
         mds=1
 ...
 2       k=ng**ndim_cr
         ...
         calls=dble(npg*k)
         dxg=one/ng
         dv2g=(calls*dxg**ndim_cr)**2/dble(npg)/dble(npg)/dble(npg-one)
         xnd=dble(nd)
         ndm=nd-1
         dxg=dxg*xnd
         xjac=one/calls
         do 3 j=1,ndim_cr
         dx(j)=xu_cr(j)-xl_cr(j)
 3       xjac=xjac*dx(j)
c
...
c--- read-in grid if necessary
 ...
 7       xi(nd,j)=one
         ndo=nd
c
 8       if(nprn_cr.ge.0)write(6,200)ndim_cr,calls,it,itmx,acc_cr
     1   ,mds,nd,(xl_cr(j),xu_cr(j),j=1,ndim_cr)
         call flush(6)
c
...
c
 ...
 28     xi(nd,j)=one
c
        if(it.lt.itmx.and.acc_cr*dabs(avgi).lt.sd)go to 9
...
c--- write-out grid if necessary
 ...
 200    format(/ 1X,' Input parameters for vegas:  ndim_cr=',i3,
     1  '   ncall_cr=',f8.0/28x,'  it=',i5,'    itmx=',i5/28x,
     2  '  acc_cr=',g9.3/28x,'  mds=',i3,'     nd=',i4/28x,
     3  '  (xl_cr,xu_cr)=',(t40,'( ',g12.6,' , ',g12.6,' )'))
