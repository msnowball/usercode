...
c---
      ...
      s(j,k)=2d0*(p(j,4)*p(k,4)
     &     -p(j,1)*p(k,1)-p(j,2)*p(k,2)-p(j,3)*p(k,3))
!       call writeout(p) 
c---set msq=0 to initialize
      ...
      s12=s(1,2)
!---- saftey ensure that s12 > wsqmin and bsqmin
      if((s12.lt.wsqmin).or.(s12.lt.bbsqmin)) return 
