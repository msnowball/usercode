!----- C. Williams Jan 2012
      ...
      subroutine gen_LO_XS(mu_rin,mu_fin,LO_xs,n_w) 
      implicit none 
...
!--------- INITIALIZE CODE 
      ...
      ncall=10000
!---------------------------------------------------------------
!  IF MET is present adjust number of dimensions accordiningly 
!----------------------------------------------------------------
      if(inc_MET) ndim_cr=ndim_cr-no_neutrinos 

!
!-------- VEGAS 
      call gen_LO_vegas(0,itmx,ncall,.false.,lo_1,err_l1)

!---------WRITE TO FILE 
      if(write_out) then 
         call gen_LO_vegas(1,10,n_w,.true.,LO_xs,err_l1)
...
!-------------- EXIT 
      ...
      subroutine gen_LO_vegas(myinit,myitmx,
...
************************************************************************
 ...
 !     write(6,*) 'Ndim in vegas =' ,ndim_Cr
      ...
      call vegasnr_cr(region,ndim_cr,genLO_int,myinit,myncall,myitmx,
     .        0,xinteg,xerr,chi)
