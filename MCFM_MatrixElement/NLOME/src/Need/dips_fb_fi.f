...
*     Author: J. M. Campbell                                           *
*     August, 1999        
!===== MODIFIED FOR NLOME BY C. Williams
...
************************************************************************
      ...
      subroutine dips_fb_fi(nd,p,ip,jp,kp,sub,subv,msq,msqv,
     . subr_born,subr_corr)
      implicit none
      include 'constants.f'
      include 'qcdcouple.f'
...
***********************************************************************
      if ((ip .le. 2) .and. (kp .le. 2)) then
         ...
         x=(sij+sjk+sik)/(sik+sjk)
         omx=one-x
         write(6,*) 'x = ',x,' 1 - x = ',omx
         write(6,*) 'sij = ',sij,' sjk = ',sjk,'sik = ',sik
...
C---transform the momenta so that only the first npart+1 are filled
         call transform_fb_fi(p,ptrans,x,ip,jp,kp)
