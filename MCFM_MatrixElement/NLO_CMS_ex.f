...
!------ write outputs 
      ...
      outfile_sig1=dir(1:idir)//datafile(1:istem)//'_sig_m4l' 
      outfile_sig2=dir(1:idir)//datafile(1:istem)//'_sig_m125'
      outfile_back=dir(1:idir)//datafile(1:istem)//'_back' 

!-------- open files      
      open(unit=22,file=inputfile,status='old',err=93) 
      open(unit=23,file=outfile_sig1,status='unknown') 
      open(unit=24,file=outfile_sig2,status='unknown') 
      open(unit=25,file=outfile_back,status='unknown') 
...
!---- setup Higgs mass at m4l 
 ...
 22   continue 
      ...
      open(unit=22,file=inputfile,status='old',err=33) 

!------------- Signal at m_h = 125 GeV 
...
!---- check m4l 
      ...
      call NLO_LIK(p,mu_r,mu_f,nlo_wt,nlo_err) 
      call write_cms(24,p,nlo_wt,nlo_err) 
      write(6,*) nlo_wt,nlo_err
      n_calc=n_calc+1 
      write(6,*) 
      write(6,*) 'Calculated ',n_calc,' so far ' 
...
!---- check m4l 
      ...
      call NLO_LIK(p,mu_r,mu_f,nlo_wt,nlo_err) 
      call write_cms(25,p,nlo_wt,nlo_err) 
      write(6,*) nlo_wt,nlo_err
      n_calc=n_calc+1 
      write(6,*) 
      write(6,*) 'Calculated ',n_calc,' so far ' 
